...
先使用LEA指令，偏移量为0 0011 1110，计算得到地址0x3049放入R3

再使用LDR指令，偏移量为00 0002，从R3中读取地址计算，得到地址0x3050，读取内存将0x70A2放入R4

再使用LDR指令，偏移量为00 0001，从R4中读取地址计算，得到地址0x70A3，读取内存将0x70A3放入R7

再使用LDR指令，偏移量为11 1111，从R7中读取地址计算，得到地址0x70A2，读取内存将0x70A4放入R6



若使用LEA，由于偏移量位数不足有，无法计算得到目标值
